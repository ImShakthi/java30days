package com.sakthi.hackerearth.month.easy.may;
...
public class ClusterFeature {
	...
	try {
		...
		if (clusteredPts != null && !clusteredPts.isEmpty()) {
		    ...
		    // System.out.println("N=" + n + ", x0=" + centroid.getX() +
		    // ", y0=" + centroid.getY() + ", radius="
		    // + radius + ", diameter=" + diameter);

		    // Integer res = (int) ((n * x0) + (n * y0) + (n * n *
		    // radius * radius)
		    // + (n * (n - 1) * diameter * diameter));
		    // System.out.println(res);
		    System.out.println(
			    ((n * x0) + (n * y0) + (n * n * radius * radius) + (n * (n - 1) * diameter * diameter)));
		}
    ...
    protected double calculateDiameter(final List<Point> points) {
	double diamater = 0;
	int deltaDiameter = 0;
	int deltaX;
	int deltaY;
	...
	if (deltaDiameter > 0) {
	    diamater = Math.sqrt(deltaDiameter / (points.size() * (points.size() - 1)));
    ...
    protected double calculateRadius(final List<Point> points, final int x0, final int y0) {
	double radius = 0;
	int deltaRadius = 0;
	int deltaX;
	int deltaY;
